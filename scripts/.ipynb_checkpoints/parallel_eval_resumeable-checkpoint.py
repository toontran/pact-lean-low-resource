...
def _parse_main():
    ...
    parser.add_argument("--api", default="gptf", 
        help="gptf|baseline|fairseq|gptf_neo|gptf_neo_withoutparsing")
...
class EvaluationState:
    def __init__(self, decl_files, dests):
        ...
        self.dests = {dests:False for dests in }
        self.count = 0
...
#         eval_decls_state.count = 161
#         eval_decls_state.done_dict[
#             "./test_parallel/gptf_neo_bfs2/split_decls/shard_161.names"
#         ] = False
#         eval_decls_state.done_dict[
#             "./test_parallel/gptf_neo_bfs2/split_decls/shard_162.names"
#         ] = False
    else:
    ...
    decl_files = eval_decls_state.retrieve_tasks()
    print("Declarations left for eval:", len(decl_files))
    pool_args = [(decl_file, dest, opts) for decl_file, dest in zip(decl_files, dests)]
    ...
    with ThreadPool(n_workers) as pool:
        ...
        for (decl_file, lean_cmd) in pool.imap_unordered((lambda x: eval_decls_step(*x)), pool_args):
            ...
            print(f"{eval_decls_state.count} JOBS FINISHED")
            ...
            num_shards = eval_decls_state.count
            total = 0
            num_success = 0
            for i in range(num_shards):
                shard_path = os.path.join(dest_dir,f"shard_{i}.json")
