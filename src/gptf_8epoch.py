...
#     print(f"Found checkpoints: {model_files}")
    ...
    if epochs_list and epoch in epochs_list:
        current_epoch = epoch
...
#         print(f"Checkpoint #{current_epoch} found, with min_val_loss of {min_val_loss:.3f}, resuming.")
        ...
        raise Exception(f"Cannot find epoch {epoch} at model {os.path.join(output_dir, f'{output_prefix}-[some_epoch].pt')}")
...
#                     "logprobs": {"token_logprobs": [probability]},
                    "logprobs": {"token_logprobs": [np.random.uniform(0, 1)]},
#                     "tokens": [p[0][i]],
                })
...
#                 values, prevs[i] = torch.topk(log_probs, k=1, dim=-1)
                ...
                p = prevs[i].tolist()
                if p[0][0] in special_tokens \
                       and outputs[i]["text"] not in current_texts:
                    has_end = True
                    break
                text = encoder.decode(p[0])
                value = values[0][0].item()
...
#             print(outputs)
            if has_end:
                keep_indices.append(i)
                current_texts.append(outputs[i])
...
#     print(args)
    ...
    model, current_epoch, _, _ = load_checkpoint_at_epoch(8, config, output_dir, output_prefix)
#     print(type(model) == GPT2LMHeadModel)
    ...
    while len(outputs) < 50:
        current = []
        for i in range(1):
            o = sample_sequence_budgeted(model, enc, budget=200, max_length=32, text=prompt, temperature=args.temperature, top_k=20)
            current.extend(o)
#         print(f"Found {len(current)}. Checking if parseable..", current)
#         l = check_parseable([c["text"] for c in current])
#         s = set(current)
        texts = [o["text"] for o in current]
        current = [o for i, o in enumerate(current) 
                       if (o["text"] not in texts[:i]) and ('�' not in o["text"])]
        outputs.extend(current)
    l = check_parseable([o["text"] for o in outputs])
    outputs = [outputs[i] for i in range(len(l)) if l[i]]
    outputs
    print(json.dumps({
        "model": "gptf-neo",
#             "prompt": prompt,
